...
%Fecha£º20220430
%Autor£ºSalva
%Funci¨®n: para escribir proyectos de alta puntuaci¨®n, proyecte las tres rutas generadas por multiRobots_hunts.m en el mapa satelital¡£
...
format long
 ...
 file_name='map_LZY_temp21.csv';%Introduzca la informaci¨®n de la red de carreteras
 ...
 hold on,axis on, axis equal
  ...
  % text(point_inf(index,x_index),point_inf(index,y_index),'    1plataforma no tripulada')
...
end_id=7;
   end_id0=find(end_id==point_inf(:,self_id_index));
    x=point_inf(end_id0,x_index);
   y=point_inf(end_id0,y_index);
   x=(x-minX)*kx+px;
   y=height-(y-minY)*ky+py;
   plot(x,y,'ks','LineWidth',10);
 %end